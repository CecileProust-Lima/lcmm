!==========================================================
!
!     MODULES and ROUTINES for the Optimization technique
!    (Marquardt Algorithm) in HETMIXSURV program
!
!        Cecile Proust-Lima
!
                 ...
                 if (vectbhess(i).ne.1) THEN
                    ...
                    !   if (vectbhess(j).ne.1.and.abs(b(j)).lt.1.d-2) THEN
                       if (vectbhess(j).ne.1) THEN
                          ...
                          GHG = GHG + v(m1+i)*fu1(ij1)*V(m1+j)
                          v1(ij)=fu1(ij1)
                 !         if(CurrentProcessorID.eq.0) write(*,*) 'fu1(ij)',i,j,fu1(ij1),v(m1+i),V(m1+j)
                       end if
                    end do
                 end if
              end do
              dd=GHG/dble(m-nBnonHess) 
        !      if(CurrentProcessorID.eq.0) write(*,*) 'ca =',ca,' cb =',cb,' dd =',epsd+1.d0
        !      if(CurrentProcessorID.eq.0) write(*,*) 'With partial Hessian: ca =',ca,' cb =',cb,' dd =',dd
              if (dd.lt.epsd) then 
                 istop=5 
                 v=v1
