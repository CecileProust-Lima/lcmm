...
<p align="justify"> As MMSE has a very skewed distribution, we use its normalized version easily obtained from NormPsy package (see Introduction vignette: https://cran.r-project.org/web/packages/lcmm/vignettes/introduction.html
).</p>
...
```{r, comment='', results='hide'}
m2b <- hlme(normMMSE ~ age65+I(age65^2)+CEP, random =~ age65+I(age65^2), mixture =~ age65+I(age65^2), subject = 'ID', ng = 2, data = paquid, B = c(0, 60, 40, 0, -4, 0, -10, 10, 212.869397, -216.421323,456.229910, 55.713775, -145.715516, 59.351000, 10.072221))
```
...
```{r, results='hide'}
m2c <- hlme(normMMSE ~ age65+I(age65^2)+CEP, random =~ age65+I(age65^2), mixture =~ age65+I(age65^2), subject = 'ID', ng = 2, data = paquid, B = random(m1))
```


### Grid search

<p align="justify">Finally, gridsearch function can be used to run an automatic grid search. In the next examples with $G=2$ and $G=3$ classes (`m2d`and `m3b`, respectively), `hlme` is run for a maximum of 50 iterations from 100 random vectors of initial values. The estimation procedure is then finalized only for the departure that provided the best log-likelihood after 30 iterations.</p>
