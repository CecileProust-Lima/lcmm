...
The function predictY provides by default the mean outcome value predicted for a profile of covariates (provided in newdata). This prediction (with marg=TRUE by default) is a marginal prediction. 
...
```{r, comment=''}
predictY(mhlme2, paquid[2:6,], marg=FALSE)
```

Predictions of the outcomes can also be also directly computed for a latent process value. This is useful when nonlinear link functions are used in the models. This is obtained with function predictYcond. 
