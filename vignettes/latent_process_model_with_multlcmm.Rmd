...
https://cran.r-project.org/web/packages/lcmm/vignettes/latent_process_model_with_lcmm.html
). 
...
mult_lin <- multlcmm(MMSE + IST + BVRT ~ age75 + male + time + I(time^2/10) + contrast(male)
, random =~ time +I(time^2 / 10), subject='ID', data = paquid, randomY = T, cor = BM(time))
```
...
# Example with Beta
mult_beta <- multlcmm(MMSE + IST + BVRT ~ age75 + male + time + I(time^2/10) + contrast(male), random =~ time + I(time^2/10), subject='ID', data = paquid, randomY = T, cor = BM(time), link = 'beta')
```

Or the link functions can be chosen differently. For instance,

```{r test73,message=FALSE, results='hide'}
# different number of knots in splines
mult_betaspl <- multlcmm(MMSE + IST + BVRT ~ age75 + male + time + I(time^2/10) + contrast(male)
, random =~time + I(time^2/10), subject='ID', data = paquid, randomY = T, cor = BM(time), link = c('beta','3-quant-splines','3-quant-splines'))
```
...
```{r test71,message=FALSE, results='hide'}
# with splines
mult_splines <- multlcmm(MMSE + IST + BVRT ~ age75 + male + time + I(time^2/10) + contrast(male)
, random =~time + I(time^2/10), subject='ID', data = paquid, randomY = T, cor = BM(time), maxiter=50, link = c('3-quant-splines'))
```
...
```{r test72,message=FALSE, results='hide'}
# with splines
mult_splines2 <- multlcmm(MMSE + IST + BVRT ~ age75 + male + time + I(time^2/10) + contrast(male)
, random =~time + I(time^2/10), subject='ID', data = paquid, randomY = T, cor = BM(time), maxiter=50, link = c('3-quant-splines'),posfix=21,B=mult_splines$best)
```
...
```{r,message=FALSE,eval=TRUE,  fig.height=6, fig.width=7}
par(mfrow=c(2,2))
plot(mult_betaspl, which="fit", var.time="time", bty="l", xlab="time since entry (in decades)", cex.lab=1.1, break.times=8, ylab="latent process", lwd=2, marg=F, ylim=c(-2,0.0), xlim=c(0.1,2), shades = TRUE, outcome = 1, col=2, main="MMSE predictions vs observations" ) 

plot(mult_betaspl, which="fit", var.time="time", bty="l", xlab="time since entry (in decades)", break.times=8, ylab="latent process", lwd=2,marg=F, ylim=c(-2,0.3), xlim=c(0.1,2), shades = TRUE, outcome = 2, col=3, cex.lab=1.1, main="IST predictions vs observations" )

plot(mult_betaspl, which="fit", var.time="time", bty="l", xlab="time since entry (in decades)", break.times=8, ylab="latent process", lwd=2, marg=F, ylim=c(-1.5,0.5), xlim=c(0.1,2), shades = TRUE, outcome = 3, col=4, cex.lab=1.1, main="BVRT predictions vs observations" ) 
...
The latent process mixed model for multivariate markers extends to the heterogeneous case with latent classes. The same strategy as explained with hlme (see https://cran.r-project.org/web/packages/lcmm/vignettes/latent_class_model_with_hlme.html
) can be used.  


## joint analysis of a time to event

The latent process mixed model for multivariate markers extends to the case of a joint model. This is done in Jointlcmm (see
https://cran.r-project.org/web/packages/lcmm/vignettes/joint_latent_class_model_with_Jointlcmm.html
).
