...
#' 
#' @export
mpjord <- function(longitudinal,subject,classmb,ng,survival,
                    ...
                    prior,logscale=FALSE,subset=NULL,na.action=1,posfix=NULL,partialH=FALSE,verbose=TRUE,methInteg="MCO",nMC=1000)
    {
        ...
        if(!(methInteg %in% c("MCO","MCA","QMC"))) stop("'methInteg' should be either 'MCO', 'MCA', or 'QMC'")
        ...
        for(k in 1:K)
            {
                ...
                {
                    if(all(longitudinal[[k]]$linktype!=3))
                    {
        ...
        else
        {
            ...
            nbevt <- length(which(names(table(data[,nom.Event]))>0))
        ...
        for(k in 1:K)
            {   
                ...
                {
                    formk <- paste(formk,as.character(mod$call$random)[2],sep="+")
                }
                ...
                if(contrainte==2)
                {
                    ...
                    for (m in 1:ny[k])
                    {
                        ...
                        if(mod$linktype[m]==3) ntr[sum(ny[1:k])-ny[k]+m] <- mod$nbmod[m]-1
                    }
                }
                if(contrainte==1)
                {
                    ...
                    epsY[k] <- ifelse(is.null(mod$epsY),0,mod$epsY)
                    ntr[k] <- switch(idlink+1,2,4,nbzitr[k]+2,sum(mod$ide))
                }
            }
...
###uniqueY0 et indiceY0
    ...
    for (k in 1:K)
    {
        for (m in 1:ny[k])
        {
            if((idlink[sum(ny[1:k])-ny[k]+m]!=2) & (idlink[sum(ny[1:k])-ny[k]+m]!=3))
            {
            ...
            if(length(as.vector(table(ym)))==length(uniqueTemp))
            {
                ...
                if(idlink[sum(ny[1:k])-ny[k]+m]==2){ indiceTemp <- nb + indice[permut]} else {indiceTemp <- indice[permut]}
                
                nb <- nb + length(uniqueTemp)
                uniqueY0 <- c(uniqueY0, uniqueTemp)
                indiceY0 <- c(indiceY0, indiceTemp)
                nvalSPLORD0 <- c(nvalSPLORD0,length(uniqueTemp))
            }
            else
            {
                uniqueY0 <- ym
                indiceY0 <- c(1:length(ym))
                nvalSPLORD0 <- c(nvalSPLORD0,length(ym))
            }
        }
    }
...
###ordonner les mesures par individu
        ...
        for  (k in 1:K)
        {
          nk <- table(matYXord[which(matYXord$processK==k),1]) 
          if(max(nk)>maxmes) maxmes <- max(nk)
          
            for (m in 1:ny[k])
            {
                #nmesM[,sum(ny[1:k])-ny[k]+m] <- table(matYXord[which(matYXord$processK==k & matYXord$outcomeM==m),1]) # marche pas si certains ont 0 mesure
              nmesM[,sum(ny[1:k])-ny[k]+m] <- sapply(unique(IND),function(x,matYXord){length(matYXord[which(matYXord[,1]==x & matYXord$processK==k & matYXord$outcomeM==m),1])},matYXord=matYXord)
            }
        }
        ...
        if(methInteg==3) 
          {
            maxdim <- max(q)+max(nalea)
            if(any(ncor>0)) maxdim <- maxdim+maxmes
            if(maxdim>0) seqMC <- randtoolbox::sobol(n=nMC,dim=maxdim,normal=TRUE,scrambling=1) 
          }
...
#return(b)
        ## pour reduire le nb d'arguments (65 max):
        int4 <- c(idtrunc,logspecif,maxiter,contrainte)
        convBLG <- c(convB,convL,convG)
        haz <- c(typrisq,risqcom)
        MC <- c(methInteg,nMC)
        
        #browser()
        out <- .Fortran(C_mpjord,
                        as.integer(K),
                        as.integer(ny),
                        as.integer(nbevt),
                        as.integer(ng),
