...
#' @param  tname the name of the variable represneting the measurement times in the object.
#' Default to the second column's name of times if it is a data frame, and to object$var.time otherwise.
#' @param n number of subject to simulate. Required only if times is not a data frame
...
#' @param entry expression to simulate a subject's entry time
#' @param dropout expression to simulate a subject's time to dropout
...
#' @return a data frame with one line per observation and one column per variable.
#'
#' @author Viviane Philipps and Cecile Proust-Lima
#'
#'@export
simulate.lcmm <- function(object, nsim, seed, times, tname=NULL, n,
                          Xbin=NULL, Xcont=NULL, entry=NULL,
